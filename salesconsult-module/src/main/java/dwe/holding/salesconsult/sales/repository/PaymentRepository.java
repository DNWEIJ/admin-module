...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("SELECT sum(p.amount) FROM Payment as p  WHERE p.customerId = :customerId and p.paymentDate <= :date and p.memberId = :memberId")
    ...
    @Query("SELECT sum(p.amount) FROM Payment as p WHERE p.customerId = :customerId and p.memberId = :memberId")
    BigDecimal getSumAmountOfPayment(@Param("customerId") Long customerId, @Param("memberId") Long memberId);

    @Query("SELECT max(p.paymentDate) FROM Payment as p WHERE p.customerId = :customerId and p.memberId = :memberId")
