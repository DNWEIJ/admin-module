...
public class OTCSellController {
    ...
    String setupProductSell(@NotNull @PathVariable Long customerId, @NotNull @PathVariable Long appointmentId, @NotNull @PathVariable Long petId, Model model) {
        ...
        HashSet<Long> ids = app.getVisits().stream().map(Visit::getPet).map(Pet::getId).collect(Collectors.toCollection(HashSet::new));
        model
                ...
                .addAttribute("pets", customer.pets().stream().filter(pet -> !ids.contains(pet.id()) && !pet.deceased()))
                .addAttribute("deceasedPets", customer.pets().stream().filter(pet -> !ids.contains(pet.id()) && pet.deceased()));
        return "sales-module/otc/productpage";
    }

    @PostMapping("/otc/search/{customerId}/sell/{appointmentId}/{petId}")
    String foundProductHtmx(@PathVariable Long customerId, @PathVariable Long appointmentId, @PathVariable Long petId,
                            Double inputCostingAmount, String inputBatchNumber, Long inputCostingId, String spillageName,
        ...
        model.addAttribute("lineItems", newLineItems);
        return "costinSearch";
    }
